...
P_GAIN             = 2.0  # how aggressively to correct heading errors
MAX_HEADING_ADJUST = 20   # cap on heading correction (%)
LOOP_INTERVAL_MS   = 20   # how often to update the loop (ms)

# The desired heading is straight ahead, which is 0 degrees on the IMU.
# hub.imu.heading() starts at 0 when the program starts, so no reset needed.
TARGET_HEADING = 0  # degrees (positive = clockwise)
...
while True:
    ...
    # Measure how far each wheel has rotated (degrees, positive = forward)
    ...
    # Read the IMU heading (degrees, positive = clockwise = veering right)
    ...
    # If heading_error > 0 (robot veered right), we slow the right motor
    # and speed up the left motor to turn back left.
